    ...
    public function __construct()
   {
    ...
    {     
        ...
        if($request->ajax())
        {
            ...
            $users = User::with('user_wallets')->has('user_wallets')->get();
            $total2Pay = 0;
            foreach($users as $user) {
                $credit = $user->user_wallets->sum('credit');
                $debit = $user->user_wallets->sum('debit');
                $toPay = ($credit-$debit);
                if ($toPay<0) {
                    $total2Pay = $total2Pay + abs($toPay);
                }
            }
            ...
            $report['total_2_pay'] = number_format($total2Pay,2);                 
    ...
    {   
        ...
        {          
            ...
            $customers->map(function ($customer) use ($request) { 
                    ...
                    $admin = Auth::user();
                    if ($admin->hasRole(2)) {
                        $customer_orders = $customer->transactions->where('admin_id', $admin->id)->where('created_at', '>=' , date('Y-m-d', strtotime($request->from_date)).' 00:00:00')->where('created_at', '<=' , date('Y-m-d', strtotime($request->to_date)).' 23:59:59');  
                    } else {
