...
if (! function_exists('totalStockWorth')) {
    function totalStockWorth()
    {
        ...
        foreach($products as $product) {
            $total = $total + $product->store_products->sum('quantity')*$product->cost;
        }
...
if (! function_exists('getDefaultVariant')) {
    ...
    {
        $product_default_variant = Product::where(['product_id' => $product_id, 'is_default' => 1])->orderBy('id','asc')->first();
...
if (! function_exists('adminDetails')) {
    function adminDetails()
    {
        ...
        <p> 
            ...
            <b>BANK NAME:</b> Cash Plus<br>
            <b>ACCOUNT NAME:</b> VAPESONIC<br>
            <b>ACCOUNT NO:</b> 08766758<br>
            <b>SORT CODE:</b> 08-71-99
        </p>';
    }
}
...
if (! function_exists('sixDigitInvoiceNumber')) {
    function sixDigitInvoiceNumber($orderId, $invoiceNo = null)
    {
        if ($invoiceNo != null) {
            return $invoiceNo;
        }
...
if(! function_exists('getVatCharges')){
    function getVatCharges(){
        ...
        if ($vatCharge) {
            ...
            $vatCharge=(float)$vatCharge->rate;
            return $vatCharge;
        }
        return 0;
    }
...
if(! function_exists('getVatName')){
    function getVatName(){
        $vat = TaxRate::select('name')->where('id', settingValue('tax_id'))->first();
        if ($vat) {
            return $vat->name;
        }
        return 'VAT @ 20%';
...
if(!function_exists('getAllCategoryForList')){
    function getAllCategoryForList(){
